...
//
// Most of this file addresses missing CellClass::RecalcAttributes calls:
// RecalcAttributes derives LandType/Passability from a cell's iso tile and
// overlay. Several game code paths clear overlays (crates, flags, veinholes,
// bridges) without calling RecalcAttributes, leaving LandType stale. This
// diverges between players when an aircraft passes over the cell(s) which
// later leads to pathfinding differences.
//
// The final section addresses a separate desync where a view-dependent
// function (TechnoClass::Visual_Character) gates the creation of a
// synchronized object (the "Behind" AnimClass).

#include <Utilities/Macro.h>
#include <FootClass.h> // must precede Cast.h (via TechnoClass.h) so FootClass is complete
#include <CellClass.h>
#include <MapClass.h>
...
// Fixes an issue with the game's trainer check for map reveal.
// Temporary until next Phobos release which contains a proper fix for this.
...
// ============================================================
// Invisible techno "Behind" anim desync fix
// ============================================================
// TechnoClass::AI (0x6F9E50) calls TechnoClass::Draw_Behind (0x70F1D0) every
// frame for any CanBeHidden techno whose center cell is covered by a building
// (CellClass_is_covered, 0x487E00). Draw_Behind creates Rule->Behind as a
// *synchronized* AnimClass, gated on TechnoClass::Visual_Character() returning
// VISUAL_NORMAL.
//
// For an Invisible=yes techno, Visual_Character (0x703860) returns
// VISUAL_NORMAL only on the machine of the player that owns it (the
// "IsInvisible && IsOwnedByPlayer" branch) and VISUAL_HIDDEN on every other
// client. This is the only path through Visual_Character that returns
// VISUAL_NORMAL in a client-dependent way.
//
// The desync is not the lamp's own fields (the sync CRC hashes only a few
// select fields, not IsInvisible/BehindAnim). It is that the BehindAnim is a
// *synchronized* object created on one client only: AnimClass::AnimClass ->
// AbstractClass::Create_ID -> ScenarioClass::Increment_UniqueID bumps the
// shared Scen->UniqueID counter (and the ctor also draws Scen->RandomNumber if
// the anim has RandomRate). So from that frame on, every newly created object
// gets a different ID (and RNG roll) across clients, diverging the fields the
// CRC does hash on many unrelated objects.
//
// It triggers when a building is placed/deployed over a player-owned invisible
// object - e.g. an INBLULMP lamp that a trigger reassigned from Neutral to a
// player, then a deployed MCV's construction yard covers its cell.
//
// Invisible technos never draw a sprite, so the "behind" marker is meaningless
// for them. Skip it on every client to keep the decision deterministic.
//
// Hook sits right after the Visual_Character call returns (EAX = result,
// ESI = this), replacing the original "test eax,eax / jnz 0x70F659" gate.
